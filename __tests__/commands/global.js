...
if (isCI) {
  test.concurrent('add without flag', (): Promise<void> => {
    return runGlobal(
      ...
      async config => {
        expect(
          await fs.exists(
            path.join(config.globalFolder, 'node_modules', 'react-native-cli'),
          ),
        ).toEqual(true);
        expect(
          await fs.exists(
  ...
  test.concurrent('add with prefix flag', (): Promise<void> => {
    ...
    return runGlobal(
      ...
      async config => {
        expect(
          await fs.exists(getGlobalPath(tmpGlobalFolder, 'react-native')),
        ).toEqual(true);
      },
    );
  });
...
test.concurrent('add', async (): Promise<void> => {
  ...
  return runGlobal(
    ...
    async config => {
      expect(
        await fs.exists(
          path.join(tmpGlobalFolder, 'node_modules', 'react-native-cli'),
        ),
      ).toEqual(true);
    },
  );
