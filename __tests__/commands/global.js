...
if (process.platform === 'win32') {
  test.concurrent("shouldn't create shims of shims on Windows", async (): Promise<void> => {
    const tmpGlobalFolder = await createTempGlobalFolder();
    const tmpPrefixFolder = await createTempPrefixFolder();
    const flags = {globalFolder: tmpGlobalFolder, prefix: tmpPrefixFolder};
    return runGlobal(['add', 'react-native-cli'], flags, 'add-with-prefix-flag', async config => {
      ...
      expect(await fs.exists(path.join(tmpPrefixFolder, 'bin', 'react-native.cmd'))).toEqual(true);
      expect(await fs.exists(path.join(tmpPrefixFolder, 'bin', 'react-native.ps1'))).toEqual(true);

      // Should not have shims of shims
      ...
      expect(await fs.exists(path.join(tmpPrefixFolder, 'bin', 'react-native.cmd.cmd'))).toEqual(false);

      // PowerShell shims should directly reference node, not sh or pwsh
      expect(await fs.readFile(path.join(tmpPrefixFolder, 'bin', 'react-native.ps1'))).toContain('node$exe');
    });
  });
}
