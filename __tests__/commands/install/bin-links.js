...
test('install should hoist nested bin scripts', (): Promise<void> => {
  return runInstall({binLinks: true}, 'install-nested-bin', async config => {
    const binScripts = await fs.walk(
      path.join(config.cwd, 'node_modules', '.bin'),
    );
    ...
    expect(await linkAt(config, 'node_modules', '.bin', 'standard')).toEqual(
      '../standard/bin/cmd.js',
    );
    expect(await linkAt(config, 'node_modules', '.bin', 'eslint')).toEqual(
...
test('direct dependency bin takes priority over transitive bin', (): Promise<
  void,
> => {
...
test.concurrent('install should respect --no-bin-links flag', (): Promise<
  void,
> => {
  return runInstall({binLinks: false}, 'install-nested-bin', async config => {
    const binExists = await fs.exists(
      path.join(config.cwd, 'node_modules', '.bin'),
    );
...
test('newer transitive dep is overridden by newer direct dep', (): Promise<
  void,
> => {
  return runInstall(
    {binLinks: true},
    'install-bin-links-newer',
    async config => {
      expect(await linkAt(config, 'node_modules', '.bin', 'eslint')).toEqual(
        '../eslint/bin/eslint.js',
      );
...
test('newer transitive dep is overridden by older direct dep', (): Promise<
  void,
> => {
  return runInstall(
    {binLinks: true},
    'install-bin-links-older',
    async config => {
      expect(await linkAt(config, 'node_modules', '.bin', 'eslint')).toEqual(
        '../eslint/bin/eslint.js',
      );
...
test('first transient dep is installed when same level and reference count', (): Promise<
  void,
> => {
  return runInstall(
    {binLinks: true},
    'install-bin-links-conflicting',
    async config => {
      expect(await linkAt(config, 'node_modules', '.bin', 'eslint')).toEqual(
        '../eslint/bin/eslint.js',
      );
...
test('first dep is installed when same level and reference count and one is a dev dep', (): Promise<
  void,
> => {
  return runInstall(
    {binLinks: true},
    'install-bin-links-conflicting-dev',
    async config => {
      expect(await linkAt(config, 'node_modules', '.bin', 'eslint')).toEqual(
        '../eslint/bin/eslint.js',
      );
