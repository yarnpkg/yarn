...
async function mockConstants(
  base: Config,
  mocks: Object,
  cb: (config: Config) => Promise<void>,
): Promise<void> {
  ...
  await cb(
    await require('../../../src/config.js').default.create(opts, base.reporter),
  );
...
test.concurrent('properly find and save build artifacts', async () => {
  await runInstall({}, 'artifacts-finds-and-saves', async (config): Promise<
    void,
  > => {
    const integrity = await fs.readJson(
      path.join(config.cwd, 'node_modules', constants.INTEGRITY_FILENAME),
    );

    expect(integrity.artifacts['dummy@0.0.0']).toEqual([
      'dummy',
      path.join('dummy', 'dummy.txt'),
      'dummy.txt',
    ]);

    // retains artifact
    const moduleFolder = path.join(config.cwd, 'node_modules', 'dummy');
    expect(await fs.readFile(path.join(moduleFolder, 'dummy.txt'))).toEqual(
      'foobar',
    );
    expect(
      await fs.readFile(path.join(moduleFolder, 'dummy', 'dummy.txt')),
    ).toEqual('foobar');
  });
});
...
test('changes the cache path when bumping the cache version', async () => {
  await runInstall({}, 'install-github', async (config): Promise<void> => {
    ...
    expect((JSON.parse(String(inOut.read())): any).data).toMatch(
      /[\\\/]v1[\\\/]?$/,
    );

    await mockConstants(config, {CACHE_VERSION: 42}, async (config): Promise<
      void,
    > => {
      await cache(config, reporter, {}, ['dir']);
      expect((JSON.parse(String(inOut.read())): any).data).toMatch(
...
test('changes the cache directory when bumping the cache version', async () => {
  await runInstall({}, 'install-production', async (config, reporter): Promise<
    void,
  > => {
    ...
    const firstReinstall = new Install(
    ...
    expect(
      await fs.exists(
        path.join(config.cwd, 'node_modules', 'is-array', 'yarn.test'),
      ),
    ).toEqual(true);
    ...
    await mockConstants(config, {CACHE_VERSION: 42}, async (config): Promise<
      void,
    > => {
      const secondReinstall = new Install(
        {skipIntegrityCheck: true},
        config,
        reporter,
        lockfile,
...
test.concurrent(
  ...
  async () => {
    async function check(cwd: string): Promise<void> {
      ...
      // removes extraneous
      expect(await fs.exists(path.join(moduleFolder, 'dummy2.txt'))).toEqual(
        false,
      );
    }
...
test.concurrent(
  ...
  async () => {
    await runInstall(
      {production: true},
      'install-prod-deduped-dev-dep',
      async config => {
        expect(
          (await fs.readJson(
            path.join(config.cwd, 'node_modules', 'a', 'package.json'),
          )).version,
        ).toEqual('1.0.0');

        expect(
          (await fs.readJson(
            path.join(config.cwd, 'node_modules', 'c', 'package.json'),
          )).version,
        ).toEqual('1.0.0');
...
test.concurrent(
  ...
  async () => {
    await runInstall(
      {production: true},
      'install-prod-deduped-direct-dev-dep',
      async config => {
        expect(
          (await fs.readJson(
            path.join(config.cwd, 'node_modules', 'a', 'package.json'),
        ...
        expect(
          (await fs.readJson(
            path.join(config.cwd, 'node_modules', 'b', 'package.json'),
          )).version,
        ).toEqual('1.0.0');
...
test.concurrent('hoisting should factor ignored dependencies', async () => {
  ...
  async function checkNormal(config): Promise<void> {
    ...
    expect(
      (await fs.readJson(
        path.join(config.cwd, 'node_modules', 'd', 'package.json'),
      )).version,
    ).toEqual('1.0.0');
  ...
  await runInstall(
    {},
    'install-ignored-retains-hoisting-structure',
    async config => {
...
test.concurrent(
  ...
  () => {
    return runInstall(
      {production: true, binLinks: true},
      'install-production-bin',
      async config => {
        expect(
          await fs.exists(
            path.join(config.cwd, 'node_modules', '.bin', 'touch'),
          ),
        ).toEqual(false);

        expect(
          await fs.exists(
            path.join(config.cwd, 'node_modules', '.bin', 'rimraf'),
          ),
        ).toEqual(true);
      },
    );
...
test.concurrent('install file: protocol with relative paths', (): Promise<
  void,
> => {
  return runInstall(
    {noLockfile: true},
    ...
    async config => {
      expect(
        await fs.readFile(
          path.join(config.cwd, 'node_modules', 'root-a', 'index.js'),
        ),
      ).toEqual('foobar;\n');
    },
  );
...
test.concurrent(
  ...
  async (): Promise<void> => {
    ...
    const compLoc = path.join(
      fixturesLoc,
      'install-file-without-cache',
      'comp',
      'index.js',
    );
    ...
    await runInstall(
      ...
      async (config, reporter) => {
        expect(
          await fs.readFile(
            path.join(config.cwd, 'node_modules', 'comp', 'index.js'),
          ),
        ).toEqual('foo\n');
  ...
  async (): Promise<void> => {
    await runInstall(
      ...
      async (config, reporter) => {
        ...
        expect(
          await fs.readFile(
            path.join(config.cwd, 'node_modules', 'a', 'index.js'),
          ),
        ).toEqual('foo;\n');

        expect(
          await fs.readFile(
            path.join(config.cwd, 'node_modules', 'b', 'index.js'),
          ),
        ).toEqual('bar;\n');
      },
    );
...
test.concurrent('install file: protocol', (): Promise<void> => {
  return runInstall({noLockfile: true}, 'install-file', async config => {
    expect(
      await fs.readFile(
        path.join(config.cwd, 'node_modules', 'foo', 'index.js'),
      ),
    ).toEqual('foobar;\n');
  });
});
...
test.concurrent('install everything when flat is enabled', (): Promise<
  void,
> => {
  return runInstall(
    {noLockfile: true, flat: true},
    'install-file',
    async config => {
      expect(
        await fs.readFile(
...
test.concurrent('install renamed packages', (): Promise<void> => {
  return runInstall({}, 'install-renamed-packages', async (config): Promise<
    void,
  > => {
    ...
    const json2 = await fs.readJson(
      path.join(dir, 'left-pad2', 'package.json'),
    );
...
test.concurrent('install from offline mirror', (): Promise<void> => {
  return runInstall({}, 'install-from-offline-mirror', async (config): Promise<
    void,
  > => {
    ...
    expect(
      allFiles.findIndex((file): boolean => {
        return (
          file.relative ===
          path.join('node_modules', 'fake-dependency', 'package.json')
        );
      }),
...
test.concurrent('install from git cache', (): Promise<void> => {
  return runInstall({}, 'install-from-git-cache', async (config): Promise<
    void,
  > => {
...
test.concurrent(
  ...
  (): Promise<void> => {
    return runInstall({}, 'scripts-order', async (config, reporter) => {
      expect(
        await fs.exists(
          path.join(config.cwd, 'node_modules/dep-a/dep-a-built'),
        ),
      ).toBe(true);
      expect(
        await fs.exists(
          path.join(config.cwd, 'node_modules/dep-b/dep-b-built'),
        ),
      ).toBe(true);
      expect(
        await fs.exists(
          path.join(config.cwd, 'node_modules/dep-c/dep-c-built'),
        ),
      ).toBe(true);
    });
  },
  ...
  (): Promise<void> => {
    return runInstall(
      ...
      async (config, reporter) => {
        ...
        expect(
          await fs.exists(
            path.join(config.cwd, 'node_modules/dep-d/dep-d-built'),
          ),
        ).toBe(true);
      },
    );
...
test.concurrent('install should circumvent circular dependencies', (): Promise<
  void,
> => {
  return runInstall(
    {},
...
test.concurrent('install should resolve circular dependencies 2', (): Promise<
  void,
> => {
  return runInstall(
    {},
    'install-should-circumvent-circular-dependencies-2',
    async (config, reporter) => {
...
test.concurrent(
  ...
  (): Promise<void> => {
    return runInstall({}, 'install-import-pr', async config => {
      ...
      expect(
        semver.satisfies(await getPackageVersion(config, 'mime-db'), '~1.0.1'),
      ).toEqual(true);
      expect(await getPackageVersion(config, 'fake-yarn-dependency')).toEqual(
        '1.0.1',
      );
  ...
  (): Promise<void> => {
    ...
    return runInstall(
      ...
      async (config, reporter): Promise<void> => {
        ...
        await fs.copy(
          path.join(config.cwd, 'package.json.after'),
        ...
        const lockFileWritten = await fs.readFile(
          path.join(config.cwd, 'yarn.lock'),
        );
        ...
        const mirror = await fs.walk(
          path.join(config.cwd, 'mirror-for-offline'),
        );
        expect(mirror).toHaveLength(4);

        const newFilesInMirror = mirror.filter((elem): boolean => {
          return (
            elem.relative !== 'mime-db-1.0.3.tgz' &&
            elem.relative !== 'mime-types-2.0.0.tgz'
          );
        });
...
test.concurrent('offline mirror can be enabled from parent dir', (): Promise<
  void,
> => {
  const fixture = {
  ...
  return runInstall({}, fixture, async (config, reporter) => {
    ...
    expect(
      await fs.exists(
        path.join(config.cwd, '../offline-mirror/mime-types-2.1.14.tgz'),
      ),
    ).toBe(true);
  });
});
...
test('install a scoped module from authed private registry', (): Promise<
  void,
> => {
  return runInstall(
    {noLockfile: true},
    ...
    async config => {
      ...
      expect(authedRequests[0].url).toEqual(
        'https://registry.yarnpkg.com/@types%2flodash',
      );
      expect(authedRequests[0].headers.authorization).toEqual('Bearer abc123');
      expect(authedRequests[1].url).toEqual(
        'https://registry.yarnpkg.com/@types/lodash/-/lodash-4.14.37.tgz',
      );
...
test('install a scoped module from authed private registry with a missing trailing slash', (): Promise<
  void,
> => {
  return runInstall(
    {noLockfile: true},
    'install-from-authed-private-registry-no-slash',
    async config => {
      const authedRequests = request.__getAuthedRequests();
...
test.concurrent(
  'install will not overwrite files in symlinked scoped directories',
  async (): Promise<void> => {
    await runInstall(
      {},
      ...
      async (config): Promise<void> => {
        ...
        expect('Symlinked scoped package test').toEqual(
          (await fs.readJson(path.join(dependencyPath, 'package.json')))
            .description,
        );
        expect(await fs.exists(path.join(dependencyPath, 'index.js'))).toEqual(
          false,
        );
      },
      async cwd => {
        ...
        await fs.symlink(
          dirToLink,
          path.join(cwd, '.yarn-link', '@fakescope', 'fake-dependency'),
        );

        await fs.mkdirp(path.join(cwd, 'node_modules', '@fakescope'));
        await fs.symlink(
          dirToLink,
          path.join(cwd, 'node_modules', '@fakescope', 'fake-dependency'),
        );
      },
    );
  },
...
test.concurrent(
  ...
  (): Promise<void> => {
    return runInstall(
      {},
      'install-scoped-package-with-subdependency-conflict',
      async (config, reporter) => {
        let allCorrect = true;
        try {
...
test.concurrent(
  ...
  (): Promise<void> => {
    return runInstall(
      {},
      'install-should-skip-incompatible-optional-dep',
      async config => {
        expect(
          await fs.exists(
            path.join(config.cwd, 'node_modules', 'dep-incompatible'),
          ),
        ).toEqual(false);
      },
    );
...
test.concurrent(
  ...
  (): Promise<void> => {
    return runInstall(
      ...
      async config => {
        expect(
          await fs.exists(path.join(config.cwd, 'node_modules', 'dep-a')),
        ).toEqual(false);
      },
    );
  },
...
test.concurrent(
  ...
  (): Promise<void> => {
    return runInstall(
      {},
      'install-should-skip-incompatible-optional-sub-dep',
      async config => {
        expect(
          await fs.exists(
            path.join(config.cwd, 'node_modules', 'dep-optional'),
          ),
        ).toEqual(true);
        expect(
          await fs.exists(
...
if (process.platform !== 'darwin') {
  test.concurrent(
    ...
    (): Promise<void> => {
      return runInstall(
        {},
        'install-should-not-skip-required-shared-deps',
        async config => {
          expect(
            await fs.exists(
              path.join(config.cwd, 'node_modules', 'deep-extend'),
            ),
          ).toEqual(true);
          expect(
            await fs.exists(path.join(config.cwd, 'node_modules', 'ini')),
          ).toEqual(true);
          expect(
            await fs.exists(
              path.join(config.cwd, 'node_modules', 'strip-json-comments'),
            ),
          ).toEqual(true);
        },
      );
...
test.concurrent(
  ...
  (): Promise<void> => {
    return runInstall(
      {},
      'should-not-install-failing-optional-deps',
      async config => {
        expect(
          await fs.exists(
            path.join(config.cwd, 'node_modules', 'optional-failing'),
          ),
        ).toEqual(false);
      },
    );
...
test.concurrent(
  ...
  (): Promise<void> => {
    return runInstall(
      {},
      'should-not-install-failing-deps-of-optional-deps',
      async config => {
        expect(
          await fs.exists(
            path.join(config.cwd, 'node_modules', 'optional-dep'),
          ),
        ).toEqual(true);
        expect(
          await fs.exists(path.join(config.cwd, 'node_modules', 'sub-failing')),
        ).toEqual(false);
      },
    );
  },
...
test.concurrent(
  ...
  (): Promise<void> => {
    return runInstall(
      {},
      'should-install-failing-optional-sub-deps',
      async config => {
        expect(
          await fs.exists(
            path.join(config.cwd, 'node_modules', 'optional-failing'),
        ...
        expect(
          await fs.exists(path.join(config.cwd, 'node_modules', 'sub-dep')),
        ).toEqual(true);
      },
    );
  },
  ...
  (): Promise<void> => {
    ...
    return runInstall(
      ...
      async config => {
        ...
        expect(await getPackageVersion(config, 'balanced-match')).toEqual(
          '0.4.2',
        );
      },
    );
...
test.concurrent(
  ...
  (): Promise<void> => {
    return runInstall({}, 'empty-os', async config => {
      expect(
        await fs.exists(path.join(config.cwd, 'node_modules', 'feed')),
      ).toEqual(true);
    });
  },
);
...
test.concurrent(
  'prunes the offline mirror after pruning is enabled',
  (): Promise<void> => {
    return runInstall({}, 'prune-offline-mirror', async (config): Promise<
      void,
    > => {
      ...
      expect(
        await fs.exists(path.join(config.cwd, `${mirrorPath}/dep-a-1.0.0.tgz`)),
      ).toEqual(false);
      expect(
        await fs.exists(path.join(config.cwd, `${mirrorPath}/dep-b-1.0.0.tgz`)),
      ).toEqual(false);
    });
  },
);
      ...
      async (config): Promise<void> => {
        ...
        expect(
          await fs.exists(
            path.join(
              config.cwd,
              `${mirrorPath}/@fakescope-fake-dependency-1.0.1.tgz`,
            ),
          ),
        ).toEqual(true, 'scoped package exists');
        expect(
          await fs.exists(
            path.join(config.cwd, `${mirrorPath}/fake-dependency-1.0.1.tgz`),
          ),
...
test.concurrent('bailout should work with --production flag too', (): Promise<
  void,
> => {
  return runInstall(
    {production: true},
    ...
    async (config, reporter): Promise<void> => {
      // remove file
      await fs.unlink(
        path.join(config.cwd, 'node_modules', 'left-pad', 'index.js'),
      );
      // run install again
      const reinstall = new Install(
        {production: true},
