    ...
    (): Promise<void> => {
      ...
      return runInstall(
        ...
        async config => {
          ...
          expect(await getPackageVersion(config, 'findup-sync/glob')).toEqual(
            '4.3.5',
          );
          expect(await getPackageVersion(config, 'inquirer')).toEqual('0.8.5');
          expect(await getPackageVersion(config, 'lodash')).toEqual('3.10.1');
          expect(await getPackageVersion(config, 'ast-query/lodash')).toEqual(
            '4.15.0',
          );
...
test.concurrent('install should hardlink repeated dependencies', (): Promise<
  void,
> => {
  // A@1
  // B@1 -> A@2
  ...
  return runInstall(
    {linkDuplicates: true},
    'hardlink-repeated-dependencies',
    async config => {
      const b_a = await fs.stat(
        path.join(config.cwd, 'node_modules/b/node_modules/a/package.json'),
      );
      const c_a = await fs.stat(
        path.join(config.cwd, 'node_modules/c/node_modules/a/package.json'),
      );
