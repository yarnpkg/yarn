...
test.concurrent(
  ...
  async (): Promise<void> => {
    await runInstall(
      {},
      'install-write-lockfile-if-not-satisfied',
      async (config): Promise<void> => {
        const lockfile = await fs.readFile(path.join(config.cwd, 'yarn.lock'));
...
test.concurrent(
  ...
  async (): Promise<void> => {
    ...
    try {
      await runInstall(
        {frozenLockfile: true},
        'install-throws-error-if-not-satisfied-and-frozen-lockfile',
        () => {},
      );
    } catch (err) {
      thrown = true;
...
test.concurrent(
  ...
  (): Promise<void> => {
    return runInstall(
      {},
      'install-optional-dep-from-lockfile',
      (config, reporter, install) => {
        expect(
          install &&
            install.resolver &&
            install.resolver.patterns['fsevents@^1.0.0'],
        ).toBeTruthy();
      },
    );
  },
...
# changed the file, integrity should be fine
    `;
      ...
      await fs.unlink(
        path.join(config.cwd, 'node_modules', 'mime-types', 'package.json'),
      );
      const reinstall = new Install(
        {},
        config,
...
test.concurrent(
  ...
  (): Promise<void> => {
    return runInstall({}, 'lockfile-stability', async (config, reporter) => {
      await fs.writeFile(
        path.join(config.cwd, 'node_modules', 'yarn.test'),
        'YARN TEST',
      );
      ...
      expect(
        await fs.exists(path.join(config.cwd, 'node_modules', 'yarn.test')),
      ).toBeTruthy();
  ...
  (): Promise<void> => {
    ...
    return runInstall({}, fixture, async (config, reporter) => {
      ...
      expect(lockContent).not.toContain(
        'https://fakepath.wont.download.com/mime-db/-/mime-db-1.0.0.tgz',
      );
    });
  },
);
