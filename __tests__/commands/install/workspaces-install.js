...
test.concurrent(
  "workspaces don't work without a configuration in .yarnrc",
  (): Promise<void> => {
    return runInstall({}, 'workspaces-install-enabled', async (config): Promise<
      void,
    > => {
...
test.concurrent('installs workspaces into root folder', (): Promise<void> => {
  return runInstall({}, 'workspaces-install-basic', async (config): Promise<
    void,
  > => {
    const lockfile = await fs.readFile(path.join(config.cwd, 'yarn.lock'));
    ...
    expect(
      await fs.exists(path.join(config.cwd, 'node_modules', 'isarray')),
    ).toBe(true);
    expect(
      await fs.exists(path.join(config.cwd, 'node_modules', 'repeat-string')),
    ).toBe(true);
    expect(
      await fs.exists(path.join(config.cwd, 'node_modules', 'left-pad')),
    ).toBe(true);
    expect(
      await fs.exists(path.join(config.cwd, 'node_modules', 'right-pad')),
    ).toBe(true);
    expect(
      await fs.exists(path.join(config.cwd, 'workspace-child', 'node_modules')),
    ).toBe(false);
    expect(
      await fs.exists(path.join(config.cwd, 'workspace-child', 'yarn.lock')),
    ).toBe(false);
    expect(
      await fs.exists(
        path.join(config.cwd, 'packages', 'workspace-child-2', 'node_modules'),
      ),
    ).toBe(false);
    expect(
      await fs.exists(
        path.join(config.cwd, 'packages', 'workspace-child-2', 'yarn.lock'),
      ),
    ).toBe(false);
    expect(
      await fs.exists(
        path.join(config.cwd, 'packages', 'workspace-child-3', 'node_modules'),
      ),
    ).toBe(false);
    expect(
      await fs.exists(
        path.join(config.cwd, 'packages', 'workspace-child-3', 'yarn.lock'),
      ),
    ).toBe(false);
  });
});
...
test.concurrent(
  'install should fail if a workspace has a conflicting version of a dependency',
  async (): Promise<void> => {
    let thrown = false;
    try {
