...
test.concurrent(
  ...
  (): Promise<void> => {
    return runWhy({}, ['mime-types'], 'basic', (config, reporter) => {
      ...
      expect(report[report.length - 1].data).toEqual(
        reporter.lang('whySpecifiedSimple', 'dependencies'),
      );
    });
  },
);
...
test.concurrent(
  ...
  (): Promise<void> => {
    return runWhy({}, ['left-pad'], 'basic', (config, reporter) => {
      const report = reporter.getBuffer();
      expect(report[report.length - 1].data).toEqual(
        reporter.lang('whySpecifiedSimple', 'devDependencies'),
      );
    });
  },
);
...
test.concurrent(
  ...
  (): Promise<void> => {
    return runWhy({}, ['mime-db'], 'basic', (config, reporter) => {
      const report = reporter.getBuffer();
      expect(report[report.length - 1].data).toEqual(
        reporter.lang('whyDependedOnSimple', 'mime-types'),
      );
    });
  },
);
...
test.concurrent(
  ...
  (): Promise<void> => {
    return runWhy({}, ['glob#minimatch'], 'basic', (config, reporter) => {
      const report = reporter.getBuffer();
      expect(report[report.length - 2].data).toEqual(
        reporter.lang('whyHoistedTo', 'glob#minimatch'),
      );
    });
  },
);
