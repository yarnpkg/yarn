...
test('run version with no arguments and --new-version flag', (): Promise<
  void,
> => {
  return runRun(
    [],
    {newVersion, gitTagVersion},
    'no-args',
...
test('run version with no arguments, --new-version flag where version is same as pkg.version', (): Promise<
  void,
> => {
  return runRun(
    [],
    {newVersion, gitTagVersion},
    'no-args-same-version',
...
test('run version and make sure all lifecycle steps are executed', (): Promise<
  void,
> => {
  return runRun(
    [],
...
test('run version and make sure only the defined lifecycle steps are executed', (): Promise<
  void,
> => {
  return runRun(
    [],
