...
test.concurrent('works with no arguments', (): Promise<void> => {
  return runOutdated([], {}, 'no-args', (config, reporter, out): ?Promise<
    void,
  > => {
...
test.concurrent('hides when current > latest (next, beta tag)', (): Promise<
  void,
> => {
  return runOutdated(
    [],
...
test.concurrent('shows when wanted > current and current > latest', (): Promise<
  void,
> => {
  return runOutdated(
    [],
    {},
    'wanted-newer-than-current',
    (config, reporter, out): ?Promise<void> => {
      const json: Object = JSON.parse(out);
