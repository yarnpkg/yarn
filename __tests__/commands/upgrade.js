...
test.concurrent('works with single argument', (): Promise<void> => {
  return runUpgrade(
    ...
    async (config): ?Promise<void> => {
      ...
      expect(
        lockfile.indexOf('max-safe-integer@^1.0.1:'),
...
test.concurrent('works with multiple arguments', (): Promise<void> => {
  return runUpgrade(
    ...
    async (config): ?Promise<void> => {
      ...
      expect(
        lockfile.indexOf('is-negative-zero@^1.0.0:'),
      ).toBeGreaterThanOrEqual(0);
...
test.concurrent('respects dependency type', (): Promise<void> => {
  return runUpgrade(
    ...
    async (config): ?Promise<void> => {
      ...
      expect(
        lockfile.indexOf('max-safe-integer@^1.0.0:'),
      ).toBeGreaterThanOrEqual(0);
      expect(lockfile.indexOf('left-pad@^1.1.3:')).toBeGreaterThanOrEqual(0);
...
test.concurrent('upgrades dependency packages not in registry', (): Promise<
  void,
> => {
  ...
  return runUpgrade(
    ...
    async (config): ?Promise<void> => {
      ...
      const lockFileIncludes = sha =>
        lockfile.includes(`  resolved "${gitRemote}#${sha}"`);
      ...
      ).toEqual(false, 'Lockfile should update yarn-test-git-repo SHA.');

      expect(
        lockfile.includes(
          `"e2e-test-repo@${gitRemote}#greenkeeper/cross-env-3.1.4":`,
        ),
      ).toEqual(
        true,
      ...
      ).toEqual(true, 'Lockfile should keep latest e2e-test-repo SHA.');
    },
  );
});

test.concurrent('upgrades dev dependency packages not in registry', (): Promise<
  void,
> => {
  const packages = ['yarn-test-git-repo', 'e2e-test-repo'];
  return runUpgrade(
...
test.concurrent(
  ...
  (): Promise<void> => {
    ...
    return runUpgrade(
      packages,
      {},
      'package-not-in-registry-optional',
      async (config): ?Promise<void> => {
        const lockfile = explodeLockfile(
          await fs.readFile(path.join(config.cwd, 'yarn.lock')),
        );
...
test.concurrent(
  'upgrades peer dependency packages not in registry',
  (): Promise<void> => {
    const packages = ['yarn-test-git-repo', 'e2e-test-repo'];
    return runUpgrade(
      packages,
      {},
      'package-not-in-registry-peer',
      async (config): ?Promise<void> => {
        const lockfile = explodeLockfile(
          await fs.readFile(path.join(config.cwd, 'yarn.lock')),
        );
...
test.concurrent('can prune the offline mirror', (): Promise<void> => {
  return runUpgrade(
    ...
    async (config): ?Promise<void> => {
      ...
      expect(
        await fs.exists(path.join(config.cwd, `${mirrorPath}/dep-a-1.1.0.tgz`)),
      ).toEqual(true);
      expect(
...
test.concurrent('respects --scope flag', (): Promise<void> => {
  return runUpgrade(
    ...
    async (config): ?Promise<void> => {
      ...
      expect(
        lockfile.indexOf('"@angular-mdl/core@4.0.0":'),
      ).toBeGreaterThanOrEqual(0);
