  ...
  (): Promise<void> => {
    return execInit({yes: true}, 'init-yes', async (config): Promise<void> => {
      ...
      expect(manifest.version).toEqual(
        String(config.getOption('init-version')),
      );
      expect(manifest.license).toEqual(
        String(config.getOption('init-license')),
      );
    });
  },
);
...
test.concurrent(
  ...
  async (): Promise<void> => {
    expect('hi seb').toEqual(
      await getGitConfigInfo('some-info', () => Promise.resolve('hi seb')),
    );
  },
);
...
test.concurrent(
  ...
  async (): Promise<void> => {
    expect('').toEqual(
      await getGitConfigInfo('some-info', () => Promise.reject(Error())),
    );
  },
);
