...
test.concurrent('install with --dev flag', (): Promise<void> => {
  return runAdd(
    ...
    'add-with-flag',
    async config => {
      const lockfile = explodeLockfile(
        await fs.readFile(path.join(config.cwd, 'yarn.lock')),
      );
...
test.concurrent('add with no manifest creates blank manifest', (): Promise<
  void,
> => {
...
test.concurrent('add should ignore cache', (): Promise<void> => {
  ...
  return runAdd(
    ['left-pad@1.1.0'],
    {},
    'install-save-to-mirror-when-cached',
    async (config, reporter) => {
      ...
      expect(
        JSON.parse(await fs.readFile(path.join(config.cwd, 'package.json')))
          .dependencies,
      ).toEqual({
        'left-pad': '1.1.0',
      });
...
test.concurrent('add should not make package.json strict', (): Promise<
  void,
> => {
...
test.concurrent(
  ...
  (): Promise<void> => {
    return runAdd(
      [],
      {},
      'install-should-cleanup-when-package-json-changed-3',
      async (config, reporter) => {
        // expecting yarn check after installation not to fail

        await fs.copy(
          path.join(config.cwd, 'yarn.lock.after'),
          path.join(config.cwd, 'yarn.lock'),
          reporter,
        );
        await fs.copy(
...
test.concurrent('add with new dependency should be deterministic', (): Promise<
  void,
> => {
  ...
  return runInstall(
    ...
    async (config): Promise<void> => {
      ...
      return runAdd(['mime-db@1.23.0'], {}, fixture, async config => {
        ...
        expect(await getPackageVersion(config, 'mime-types/mime-db')).toEqual(
          '1.0.3',
        );
        ...
        expect(lockFileLines.indexOf('mime-db@~1.0.1:')).toBeGreaterThanOrEqual(
          0,
        );
        expect(lockFileLines.indexOf('mime-db@1.23.0:')).toBeGreaterThanOrEqual(
          0,
        );
        expect(
          lockFileLines.indexOf('mime-types@2.0.0:'),
    ...
    return runInstall(
      ...
      async (config): Promise<void> => {
        ...
        return runAdd(['mime-db@1.0.3'], {}, fixture, async config => {
          ...
          expect(await getPackageVersion(config, 'mime-types')).toEqual(
            '2.0.0',
          );
          ...
          expect(
            await fs.exists(
              path.join(
                config.cwd,
                'node_modules/mime-types/node-modules/mime-db',
              ),
            ),
          ).toEqual(false);
...
test.concurrent('add with offline mirror', (): Promise<void> => {
  ...
  return runAdd(
    ['is-array@^1.0.1'],
    {},
    'install-with-save-offline-mirror',
    async config => {
      ...
      const rawLockfile = await fs.readFile(
        path.join(config.cwd, constants.LOCKFILE_FILENAME),
      );
...
test.skip('add-then-install git+ssh from offline mirror', (): Promise<void> => {
  ...
  return runAdd(
    ...
    async (config, reporter): Promise<void> => {
      expect(
        semver.satisfies(await getPackageVersion(config, 'mime-db'), '1.24.0'),
      ).toEqual(true);
      ...
      const newLockFileWritten = await fs.readFile(
        path.join(config.cwd, 'yarn.lock'),
      );
...
test.concurrent('install with --save and without offline mirror', (): Promise<
  void,
> => {
  const mirrorPath = 'mirror-for-offline';
  return runAdd(
    ['is-array@^1.0.1'],
    {},
    'install-with-save-no-offline-mirror',
    async config => {
      const allFiles = await fs.walk(config.cwd);
...
test.concurrent('upgrade scenario', (): Promise<void> => {
  ...
  return runAdd(
    ...
    async (config, reporter): Promise<void> => {
      ...
      const lockFileWritten2 = await fs.readFile(
        path.join(config.cwd, 'yarn.lock'),
      );
...
test.concurrent('upgrade scenario 2 (with sub dependencies)', (): Promise<
  void,
> => {
...
test.concurrent(
  ...
  (): Promise<void> => {
    ...
    return runAdd(
      [],
      {},
      'no-mirror-remote-when-duplicates',
      async (config): Promise<void> => {
        const mirrorPath = 'mirror-for-offline';

        // check that which module was downloaded to mirror
        const mirror = await fs.walk(path.join(config.cwd, mirrorPath));
        const whichModule = mirror.find((elem): any =>
          elem.relative.match(/which-1\.2\..*\.tgz/),
        );
...
test.concurrent('add should put a git dependency to mirror', (): Promise<
  void,
> => {
  const mirrorPath = 'mirror-for-offline';
...
test.concurrent('add should store latest version in lockfile', (): Promise<
  void,
> => {
  return runAdd(
    ['max-safe-integer'],
    {},
    'latest-version-in-lockfile',
    async config => {
      const lockfile = explodeLockfile(
        await fs.readFile(path.join(config.cwd, 'yarn.lock')),
      );
...
test.concurrent('add should generate correct integrity file', (): Promise<
  void,
> => {
  return runAdd(
    ['mime-db@1.24.0'],
...
test.concurrent(
  ...
  async (): Promise<void> => {
    await runAdd(
      ...
      async (config): Promise<void> => {
        const lockfile = await promisify(fsNode.readFile)(
          path.join(config.cwd, 'package.json'),
          'utf8',
        );
      ...
      async (cwd): Promise<void> => {
        ...
        await promisify(fsNode.writeFile)(
...
test.skip(
  'add asks for correct package version if user passes an incorrect one',
  async (): Promise<void> => {
    ...
    await runAdd(
      ...
      async config => {
        ...
        expect(await getPackageVersion(config, 'is-array')).toEqual(
          chosenVersion,
        );
      },
      () => {
...
test.concurrent('install with latest tag and --offline flag', (): Promise<
  void,
> => {
  return runAdd(
    ['left-pad@latest'],
    {},
    'latest-version-in-package',
    async (config, reporter, previousAdd) => {
      config.offline = true;
      const add = new Add(
        ['left-pad@latest'],
        {},
...
test.concurrent('warns when peer dependency is not met during add', (): Promise<
  void,
> => {
  return buildRun(
    reporters.BufferReporter,
...
test.concurrent('should retain build artifacts after add', (): Promise<
  void,
> => {
  return buildRun(
    reporters.BufferReporter,
