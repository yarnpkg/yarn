...
test.concurrent('with one argument does not contain readme field', (): Promise<
  void,
> => {
...
test.concurrent(
  ...
  (): Promise<void> => {
    return runInfo(['yarn', 'readme'], {}, '', (config, output): ?Promise<
      void,
    > => {
...
test.concurrent(
  ...
  (): Promise<void> => {
    return runInfo(['yarn', 'repository'], {}, '', (config, output): ?Promise<
      void,
    > => {
...
test.concurrent(
  ...
  (): Promise<void> => {
    return runInfo(['yarn', 'unknown'], {}, '', (config, output): ?Promise<
      void,
    > => {
...
test.concurrent('reports error on invalid package names', (): Promise<void> => {
  ...
  return runInfo(
    ['YARN.invalid.package.name.YARN'],
    {},
    '',
    (config, output): ?Promise<void> => {
...
test.concurrent('reports error with too many arguments', (): Promise<void> => {
  const reporter = new reporters.ConsoleReporter({});
  return runInfo(
    ['yarn', 'version', 'extra.invalid.arg'],
    {},
    '',
    (config, output): ?Promise<void> => {
