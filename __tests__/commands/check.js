...
test.concurrent('--verify-tree should report wrong version', async (): Promise<
  void,
> => {
  let thrown = false;
  try {
...
test.concurrent(
  ...
  async (): Promise<void> => {
    ...
    try {
      await runCheck(
        [],
        {verifyTree: true, production: false},
        'verify-tree-dev',
      );
    } catch (e) {
      thrown = true;
...
test.concurrent(
  ...
  async (): Promise<void> => {
    await runCheck(
      [],
      {verifyTree: true, production: true},
      'verify-tree-dev-prod',
    );
  },
);
...
test.concurrent(
  ...
  async (): Promise<void> => {
    await runInstall(
      ...
      async (config, reporter): Promise<void> => {
        await fs.unlink(
          path.join(config.cwd, 'node_modules', '.yarn-integrity'),
        );
    ...
    await runInstall(
      ...
      async (config, reporter, install, getStdout): Promise<void> => {
        ...
        expect(getStdout()).toContain(
          'Integrity check: integrity file is not a json',
        );
      },
    );
  },
...
test.concurrent(
  ...
  resolved "https://registry.yarnpkg.com/xtend/-/xtend-4.0.1.tgz#a5c6d532be656e23db820efb943a1f04998d63af"`;
        ...
        expect(thrown).toEqual(true);
        expect(getStdout()).toContain(
          "Integrity check: Lock files don't match",
        );
      },
    );
  },
      ...
      async (config, reporter, install, getStdout): Promise<void> => {
        ...
        try {
          await checkCmd.run(
            config,
            reporter,
            {integrity: true, checkFiles: true},
            [],
          );
        } catch (e) {
          thrown = true;
      ...
      async (config, reporter, install, getStdout): Promise<void> => {
        ...
        try {
          await checkCmd.run(
            config,
            reporter,
            {integrity: true, ignoreScripts: false},
            [],
          );
        } catch (e) {
          thrown = true;
...
test.concurrent(
  ...
  async (): Promise<void> => {
    await runInstall(
      ...
      path.join('..', 'check', 'integrity-lock-check'),
      async (config, reporter, install, getStdout): Promise<void> => {
        const integrityFilePath = path.join(
          config.cwd,
          'node_modules',
          '.yarn-integrity',
        );
        ...
        expect(thrown).toEqual(true);
        expect(getStdout()).toContain(
          "Integrity check: Linked modules don't match",
        );
      },
    );
  },
  ...
  async (): Promise<void> => {
    await runInstall(
      ...
      async (config, reporter, install, getStdout): Promise<void> => {
        ...
        expect(
          await fs.exists(
            path.join(config.cwd, '.yarn-meta', '.yarn-integrity'),
          ),
        ).toEqual(true);
      },
    );
