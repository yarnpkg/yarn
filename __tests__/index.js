...
function expectAnErrorMessage(
  command: Promise<Array<?string>>,
  error: string,
): Promise<void> {
  return command
    .then(function() {
...
function expectAnInfoMessageAfterError(
  command: Promise<Array<?string>>,
  info: string,
): Promise<void> {
  return command
...
test.concurrent(
  ...
  async () => {
    const stdout = await execCommand(
      'add',
      ['--no-lockfile', 'split-lines'],
      'run-add-option-in-front',
      true,
    );
...
test.concurrent(
  'should display correct documentation link for unsupported aliases',
  async () => {
...
test.concurrent(
  'should run help of run command if --help is before --',
  async () => {
    const stdout = await execCommand(
      'run',
      ['custom-script', '--help', '--'],
...
if (process.platform !== 'win32') {
  test.concurrent(
    ...
    async () => {
      ...
      expect(stdout[stdout.length - 2]).toEqual(
...
test.concurrent('should run help for command with hyphens', async () => {
  ...
  expect(lastLines[1]).toMatch(
    /yarn generate-lock-entry --use-manifest .\/package.json/,
  );
  expect(lastLines[2]).toMatch(
    /yarn generate-lock-entry --resolved local-file.tgz#hash/,
  );
  expect(lastLines[3]).toMatch(
    /Visit https:\/\/yarnpkg.com\/en\/docs\/cli\/generate-lock-entry/,
  );
});
