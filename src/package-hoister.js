...
export default class PackageHoister {
  ...
  seed(patterns: Array<string>) {
    ...
    while (true) {
      ...
      for (const [pattern, parents] of queue) {
        const info = this._seed(pattern, false, parents);
        if (info) {
          infos.push(info);
        }
      }
  ...
  _seed(pattern: string, directRequire: boolean, parent?: HoistManifest): ?HoistManifest {
    //
    ...
    let isRequired = directRequire && !ref.ignore && !isIncompatible;
    ...
    if (parent) {
      ...
      if (!directRequire && !isIncompatible && parent.isRequired) {
        isRequired = true;
      }
  ...
  prepass(patterns: Array<string>) {
    ...
    for (const packageName of Object.keys(occurences).sort()) {
      ...
      if (mostOccurenceCount > 1) {
        this._seed(mostOccurencePattern, false);
      }
    }
  }
