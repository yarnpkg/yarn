...
export let fetcher = new MirrorFetcher.Save()
  .add(
    new CacheFetcher().add(
      new UnpackFetcher().add(
        new MirrorFetcher.Load()
          .add(new GitFetcher())
          .add(
            new ArchiveFetcher({virtualPath: `/package`})
  ...
  .add(
    new UnpackFetcher().add(
      new ArchiveFetcher({virtualPath: `/package`}).add(new FileFetcher({pathPattern: `*+(.tar.gz|.tgz)`})),
    ),
  );
