...
let formatters = {
  tree: async ({packageTree, treeSize, filter}) => {
    function exportDependencies(packageNode) {
      ...
      let dependencyTrees = packageNode.dependencies
        .sortBy(dependency => {
  ...
  list: async ({packageTree, treeSize, filter}) => {
    let locatorIdentifiers = (await getPackageTreeLocators(packageTree, {includeRoot: false}))
      .filter(packageLocator => {
    ...
    return locatorIdentifiers
      .map(locatorIdentifier => {
        return `${locatorIdentifier}\n`;
      })
      .join(``);
  },
  ...
  uniq: async ({packageTree, treeSize}) => {
    let locatorSetGroups = (await getPackageTreeLocators(packageTree, {includeRoot: false}))
      .filter(packageLocator => {
        return filter(packageLocator);
      })
    ...
    return locatorSetGroups
      .map((locatorSetGroup, count) => {
        return locatorSetGroup
          .sortBy((locatorSet, packageName) => packageName)
          .map((locatorSet, packageName) => {
...
export default concierge =>
  concierge
    ...
    .action(async args => {
      ...
      let {packageTree, treeSize, errors = new Immutable.Map()} = await uiUtils.trackProgress(
      ...
      let data = await formatters[args.format]({
        packageTree,
        treeSize,
        filter: packageLocator => {
          if (args.name && !miscUtils.stringPatternMatch(packageLocator.name, [args.name])) return false;
      ...
      if (args.pager) await streamUtils.pager(data);
