...
export default concierge => concierge
    ...
    .action(async (args) => {
        ...
        let filesPatterns = packageInfo.files.concat([ ... packageInfo.bin.values(), packageInfo.main, packageInfo.browser ]).filter(pattern => pattern).map(mapPattern);

        let walker = fsUtils.walk(packagePath, { filter: path => {

            let patternPath = Path.relative(packagePath, path).replace(/^\.\//, ``);

            if (includedPatterns.some(pattern => pattern.match(patternPath)))
                return true;

            if (ignoredPatterns.some(pattern => pattern.match(patternPath)))
                return false;

            if (packageInfo.files.size === 0 || filesPatterns.some(pattern => pattern.match(patternPath)))
                return true;
