...
export default concierge => concierge
    ...
    .command(`lock`)
    .describe(`Generate a new lockfile for your project`)
    ...
    .action(async (args) => {
        ...
        let { packageTree, errors } = await uiUtils.trackProgress({ enabled: !args.silent && args.progress, total: 1, bar: `:bar :current/:total` }, async ({ tick, add }) => {
