...
export class BaseMultiResolver extends BaseResolver {
    ...
    isSatisfied(packageLocator, availableLocator, { env }) {
        ...
        if (candidateResolvers.length === 0)
            throw new Error(`No resolver offered to handle these packages`);

        if (candidateResolvers.length > 1)
            throw new Error(`Multiple resolvers offered to resolve a same package`);
    ...
    async getCandidates(packageLocator, { env }) {
        ...
        if (candidateResolvers.length === 0)
            throw new Error(`No resolver offered to handle this package`);
