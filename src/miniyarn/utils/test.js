...
export function getPackageRegistry() {
  if (getPackageRegistry.promise) return getPackageRegistry.promise;
  ...
  return (getPackageRegistry.promise = (async () => {
    ...
    for (let packageFile of await fsUtils.walk(`${__dirname}/../../tests/packages`, {filter: [`package.json`]})) {
      ...
      if (!packageVersions.has(name)) packageVersions.set(name, new Map());
...
export async function getPackageArchivePath(name, version) {
  ...
  if (!packageEntry.has(version)) throw new Error(`Unknown version "${version}" for package "${name}"`);
...
export async function startPackageServer() {
  if (startPackageServer.url) return startPackageServer.url;
  ...
  async function processPackageInfo(params, res) {
    ...
    if (!packageEntry) return await processError(res, 404);

    let versions = Object.assign(
      {},
      ...Array.from(packageEntry.entries()).map(([version, {packageJson}]) => ({[version]: packageJson})),
    );
  ...
  async function processPackageTarball(params, res) {
    ...
    if (!packageEntry || !packageEntry.has(version)) return await processError(res, 404);
  ...
  return await new Promise((resolve, reject) => {
    let server = Http.createServer(async (req, res) => {
      if (await processPackageInfo(req.url.match(/^\/(?:(@[^\/]+)\/)?([^@\/][^\/]*)$/), res)) return;
...
export function makeTemporaryEnv(config, fn) {
  ...
  return async function() {
    ...
    await fsUtils.writeFile(
      `${path}/.yarnrc`,
      JSON.stringify({cachePath: null, mirrorPath: null, registryUrl: await startPackageServer()}),
    );
