...
export function pushIsSilent(isSilentStatus) {
  if (isSilent() && !isSilentStatus) throw new Error(`Cannot disable the silent mode via a push - use pop instead`);
...
export function trackProgress({enabled = true, bar = `:bar`, width = 50, total = 0}, callback) {
  if (isProgressEnabled() && enabled) {
    ...
    let tick = () => {
      if (progressBar.curr + 1 >= progressBar.total) progressBar.fmt += ` (:elapseds)`;
