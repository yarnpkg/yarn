...
export async function recursivelyResolvePackage(packageInfo, { resolver, fetcher, env, peers = true, production = false, tick = () => {}, add = () => {} }) {

    let dependencyKeys = [ `dependencies` ];

    if (production === false && packageInfo.devDependencies)
        dependencyKeys.push(`devDependencies`);

    if (peers === true && packageInfo.peerDependencies)
        dependencyKeys.push(`peerDependencies`);
    ...
    async function performRecursiveResolution(parentPath, packageResolution, availablePackages) {
        ...
        packageResolution = packageResolution.update(`dependencies`, dependencies => dependencies.filter(dependencyRange => {
            return !availablePackages.get(dependencyRange.name) || !resolver.isSatisfied(dependencyRange, availablePackages.get(dependencyRange.name), { env });
        }));

        // Step 2: We resolve the package dependencies to satisfiable versions

        let result = await Promise.all(packageResolution.dependencies.valueSeq().map(async (dependencyRange) => {
            return await resolvePackage(dependencyRange);
        }));

        let errors = new Immutable.Map(result.map(([ dependencyRange, error, dependencyResolution ]) => {
            return [ dependencyRange, error ];
        }).filter(([ key, value ]) => value));

        let dependencyResolutions = new Immutable.Map(result.map(([ dependencyName, error, dependencyResolution ]) => {
            return [ dependencyName, dependencyResolution ];
        }).filter(([ key, value ]) => value));

        // Step 3: We make these packages available to their own dependencies

        availablePackages = availablePackages.merge(dependencyResolutions.map(dependencyResolution => {
        ...
        let recursionResults = await Promise.all(dependencyResolutions.toList().map(async (dependencyResolution) => {
        ...
        return {
            ...
            errors: errors.concat(... recursionResults.map(({ errors }) => {
                return errors;
            })),

            packageTree: new PackageNode(packageResolution.locator).set(`dependencies`, new Immutable.Map(recursionResults.map(({ packageTree }) => {
