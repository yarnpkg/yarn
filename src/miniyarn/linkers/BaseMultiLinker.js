...
export class BaseMultiLinker extends BaseLinker {
    ...
    async link(packageNode, destination, { linker, env, limit, tick, packageInfos, handlers }) {
        ...
        if (candidateLinkers.length === 0)
            throw new Error(`No linker offered to handle this package`);

        if (candidateLinkers.length > 1)
            throw new Error(`Multiple linkers offered to link a same package`);
