...
export class BaseLinker {
  ...
  async linkDependencies(packageNode, destinationPath, {linker, env, limit, tick, packageInfos, handlers}) {
    ...
    let subLinks = await Promise.all(
      packageNode.dependencies.toList().map(async dependency => {
    ...
    let build = async ({env, tick}) =>
      await Promise.all(subLinks.map(({build}) => build({env, tick}))).then(async errorLists => {
