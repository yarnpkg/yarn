...
export async function createTemporaryFile(filePath) {
    ...
    if (filePath) {
        ...
        if (!pathUtils.isForward(filePath))
...
export async function walk(path, { filter } = {}) {
    ...
    return await new Promise((resolve, reject) => {
        ...
        let walker = klaw(path, { filter: itemPath => {
...
s
        ...
        walker.on(`data`, ({ path: itemPath }) => {
            ...
            if (!filter)
                return void(paths.push(itemPath));
...
export function packToStream(path, { virtualPath = null } = {}) {
    ...
    let packStream = tarFs.pack(path, { map: header => {
...
export class Handler {
    ...
    async steal() {
        ...
        let destination = await isDirectory(this.path)
