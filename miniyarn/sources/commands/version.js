    ...
    .action(async (args) => {
        ...
        if (!args.set && !args.bump && !args.interactive) {
            ...
            if (!packageInfo.version)
                return uiUtils.reportError(new Error(`Package has no version`));
            ...
            if (args.set)
                nextVersion = args.set;
            else if (args.bump)
                nextVersion = semver.inc(packageInfo.version, args.bump);
            else if (args.interactive)
                nextVersion = await askVersion(packageInfo.version);
...
async function askVersion(version) {
    ...
    if (version != null) {
        ...
        return await uiUtils.askOne({ default: 0, choices: [ `patch`, `minor`, `major` ].map(level => {
        ...
        return await uiUtils.askOne({ default: 0, choices: [
            { value: `0.0.1`, short: `0.0.1`, name: `0.0.1` },
            { value: `1.0.0`, short: `1.0.0`, name: `1.0.0` },
        ] });
